...
_This is the accompanying source code for the paper 'An Analytical Framework for Measuring Inequalities in the Public Opinions on Policing – Assessing COVID-19 Pandemic in Space-Time using Twitter Data', published in the Journal of Geographical Information System. Certain parts of this source code, including the calculation of opinion scores and simulation of expectation distribution, were later built into an R package named `Opitools`, which is available on `CRAN`. The building of the package happened when the article was actively under review. In the spirit of reproducibility, we will present, in this document, the actual codes that was used to conduct the original research, rather than the functions from the `Opitools` package. However, we highlight, at different sections of this document (tagged as `Opitools Package Reference: #`), different functions from the `Opitools` package that can be used to generate the same outputs as the section in question_. 
...
data_sample1 <- read.table(file=paste(WORKING_DIR, "/data/only_policeTweet_set_1.csv", sep=""), sep=",", head=TRUE)
#data_sample1 <- readRDS(file=paste(WORKING_DIR, "/data/tweets_containing_only_policing_28_69.rds", sep=""))

.
.
data_sampleN <- read.table(file=paste(WORKING_DIR, "/data/only_policeTweet_set_N.csv", sep=""), sep=",", head=TRUE)
...
#house cleaning
rm(list=ls()[! ls() %in% 
               c("WORKING_DIR", "data_1", "Pf_names_regions", "Pf_names_regions_uni", "fix.contractions", "removeSpecialChars")])
...
#--------------------------------
for(i in 1:length(Pf_names_regions_uni)){ #i=1, loop through policing regions
  ...
  for(z in 1:length(periods)){ #z = 1
    ...
    #--------------------------
    #1. Data cleaning, 
    #2. Removing duplicated text records, using similarity detection
    #3. Use only 25% of data in case of "Metropolitan Police" which is too big!
    ...
    #import keywords that defines subject B (i.e. secondary subject)
    #------------------------------------------
    ...
    #Opitools Package Reference: #1
    #-----------------------------------
    #The list of pandemic keywords can be found in 'Opitools'
    #by typing 'covid_keys' in the console after installing the package
    #-----------------------------------
    ...
    #Opitools Package Reference: #2
    #-----------------------------------
    #The above chunks included components of 
    #`opi_score` and `opi_impact` function.
    #-----------------------------------    
...
#Opitools Package Reference: #3
#-----------------------------------
#The above chunks included components of 
#`opi_score`, `opi_sim` and `opi_impact` functions.
#-----------------------------------    
...
write.table(all_Regional_afinn_combined, file=paste(WORKING_DIR, "/outputs/all_obs_exp_Sentiment_ts_1_2_3_ALL.csv", sep=""), 
            sep=",", row.names = F)
#P-values and position table for time step 1
write.table(all_pvalueS_1, file=paste(WORKING_DIR, "/outputs/all_pvalue_Sentiment_ts_1.csv", sep=""), 
            sep=",", row.names = F)
write.table(all_p_signs_1, file=paste(WORKING_DIR, "/outputs/all_sentiment_signs_ts_1.csv", sep=""), 
            sep=",", row.names = F)
#P-values and position table for time step 2
write.table(all_pvalueS_2, file=paste(WORKING_DIR, "/outputs/all_pvalue_Sentiment_ts_2.csv", sep=""), 
            sep=",", row.names = F)
write.table(all_p_signs_2, file=paste(WORKING_DIR, "/outputs/all_sentiment_signs_ts_2.csv", sep=""), 
            sep=",", row.names = F)
#P-values and position table for time step 3
write.table(all_pvalueS_3, file=paste(WORKING_DIR, "/outputs/all_pvalue_Sentiment_ts_3.csv", sep=""),
            sep=",", row.names = F)
write.table(all_p_signs_3, file=paste(WORKING_DIR, "/outputs/all_sentiment_signs_ts_3.csv", sep=""),
            sep=",", row.names = F)
...
all_UK_bing_combined <- read.table(file=paste(WORKING_DIR, "/outputs/all_obs_exp_Sentiment_ts_1_2_3_ALL_6.csv", sep=""), 
                                   sep=",", head=TRUE)

#read in 'Position' tables
all_pvalueS_1 <- read.table(file=paste(WORKING_DIR, "/outputs/all_pvalue_Sentiment_ts_1.csv", sep=""), sep=",", head=TRUE)
all_pvalueS_2 <- read.table(file=paste(WORKING_DIR, "/outputs/all_pvalue_Sentiment_ts_2.csv", sep=""), sep=",", head=TRUE)
all_pvalueS_3 <- read.table(file=paste(WORKING_DIR, "/outputs/all_pvalue_Sentiment_ts_3.csv", sep=""), sep=",", head=TRUE)

#read in P-values
all_p_signs_1 <- read.table(file=paste(WORKING_DIR, "/outputs/all_sentiment_signs_ts_1.csv", sep=""), sep=",", head=TRUE)
all_p_signs_2 <- read.table(file=paste(WORKING_DIR, "/outputs/all_sentiment_signs_ts_2.csv", sep=""), sep=",", head=TRUE)
all_p_signs_3 <- read.table(file=paste(WORKING_DIR, "/outputs/all_sentiment_signs_ts_3.csv", sep=""), sep=",", head=TRUE)
...
#Opitools Package Reference: #4
#-----------------------------------
#The above chunks form a component of 
#`opi_impact` functions.
#-----------------------------------    
...
for(y in 1:length(periods)){  #y<-1
  ...
  for(i in 1:length(unique(Pf_names_regions$Regions))){ #i=7
    ...
    for(j in 1:nrow(subsetP)) {#j=3
      ...
      UK_afinn_read <- read.table(file = paste(WORKING_DIR, "/outputs/", "police_cleaned_", 
  ...
  write.table(likert_sentimentData, file = paste(WORKING_DIR, "/outputs/Likert_Data", periods[y], ".csv", sep="_"), sep=",")
...
lik_p1 <- read.table(file = paste(WORKING_DIR, "/outputs/Likert_Data",PR[m], ".csv", sep="_"), sep=",", head=TRUE)
...
sent_pvalues_1 <- read.table(file=paste(WORKING_DIR, "/outputs/all_pvalue_Sentiment_ts_1.csv", sep=""),
                             sep=",", head=TRUE)
sent_pvalues_2 <- read.table(file=paste(WORKING_DIR, "/outputs/all_pvalue_Sentiment_ts_2.csv", sep=""), 
                             sep=",", head=TRUE)
sent_pvalues_3 <- read.table(file=paste(WORKING_DIR, "/outputs/all_pvalue_Sentiment_ts_3.csv", sep=""),
                             sep=",", head=TRUE)
...
#read `Position` files
sign_sentm_1 <- read.table(file=paste(WORKING_DIR, "/outputs/all_sentiment_signs_ts_1.csv", sep=""), sep=",", head=TRUE)
sign_sentm_2 <- read.table(file=paste(WORKING_DIR, "/outputs/all_sentiment_signs_ts_2.csv", sep=""), sep=",", head=TRUE)
sign_sentm_3 <- read.table(file=paste(WORKING_DIR, "/outputs/all_sentiment_signs_ts_3.csv", sep=""), sep=",", head=TRUE)
...
sent_scores_1_2_3 <- read.table(file=paste(WORKING_DIR, "/outputs/all_obs_exp_Sentiment_ts_1_2_3_ALL.csv", sep=""), sep=",", head=TRUE)
...
#Opitools Package Reference: #5
#-----------------------------------
#The above chunks (i.e. p-value calculation)
#form a component of `opi_impact` functions 
#-----------------------------------    
